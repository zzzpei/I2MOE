...
from src.common.utils import (
    ...
    visualize_expert_logits_simple_weight,
)

from src.ablation.SimpleWeightedAverageIMoE import SimpleWeightedAverageIMoE


def train_and_evaluate_simple_weighted_average(args, seed, fusion_model, fusion):
    ...
    ensemble_model = SimpleWeightedAverageIMoE(
    ...
    plot_total_loss_curves(
        ...
        framework="simple_weighted_average",
        fusion=fusion,
    )
    ...
    if args.save:
        ...
        Path(f"./saves/simple_weighted_average/{fusion}/{args.data}").mkdir(
            exist_ok=True, parents=True
        )

        if args.data == "mmimdb":
            save_path = f"./saves/simple_weighted_average/{fusion}/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_f1_{best_val_f1:.2f}.pth"
        else:
            save_path = f"./saves/simple_weighted_average/{fusion}/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_acc_{best_val_acc:.2f}.pth"
        torch.save(
    ...
    # visualize_sample_weights(
    #     all_routing_weights, args, framework="simple_weighted_average", fusion=fusion
    # )
