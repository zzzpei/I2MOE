...
def parse_args():
    parser = argparse.ArgumentParser(description="less_perturbed_forward-Transformer")
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/less_perturbed_forward/transformer/{args.data}",
        f"{args.data}",
        f"{args.modality}.txt",
    )
    ...
    model_kwargs = {
        "model": "less_perturbed_forward-Transformer",
        "lr": args.lr,
