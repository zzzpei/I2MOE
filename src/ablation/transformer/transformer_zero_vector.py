...
from src.ablation.train_zero_vector import (
    train_and_evaluate_zero_vector,
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="zero_vectorTransformer")
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/zero_vector/transformer/{args.data}",
        f"{args.data}",
        f"{args.modality}.txt",
    )
    ...
    model_kwargs = {
        "model": "zero_vectorMoE-Transformer",
        "lr": args.lr,
        ...
        for seed in seeds:
            ...
            if args.data == "mosi_regression":
                ...
                (
                    ...
                    test_f1_micro,
                    test_auc,
                ) = train_and_evaluate_zero_vector(args, seed, fusion_model, "transformer")
