...
from src.ablation.train_mean_vector import (
    train_and_evaluate_mean_vector,
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="mean_vectorTransformer")
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/mean_vector/transformer/{args.data}",
        f"{args.data}",
        f"{args.modality}.txt",
    )
    ...
    model_kwargs = {
        "model": "mean_vectorMoE-Transformer",
        "lr": args.lr,
        ...
        for seed in seeds:
            ...
            if args.data == "mosi_regression":
                ...
                (
                    ...
                    test_f1_micro,
                    test_auc,
                ) = train_and_evaluate_mean_vector(args, seed, fusion_model, "transformer")
