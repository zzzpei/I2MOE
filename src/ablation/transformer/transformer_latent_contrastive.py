...
from src.ablation.train_latent_contrastive import (
    train_and_evaluate_latent_contrastive,
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="latent_contrastiveTransformer")
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/latent_contrastive/transformer/{args.data}",
        f"{args.data}",
        f"{args.modality}.txt",
    )
    ...
    model_kwargs = {
        "model": "latent_contrastiveMoE-Transformer",
        "lr": args.lr,
