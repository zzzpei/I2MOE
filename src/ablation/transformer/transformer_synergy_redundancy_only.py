...
from src.ablation.train_synergy_redundancy_only import (
    train_and_evaluate_synergy_redundancy_only,
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="synergy_redundancy_only-Transformer")
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/synergy_redundancy_only/transformer/{args.data}",
        f"{args.data}",
        f"{args.modality}.txt",
    )
    ...
    model_kwargs = {
        "model": "synergy_redundancy_only-Transformer",
        "lr": args.lr,
