...
class ZeroVectorInteractionExpert(nn.Module):
    """
    Interaction Expert.
    """

    def __init__(self, fusion_model, fusion_sparse):
        super(ZeroVectorInteractionExpert, self).__init__()
    ...
    def _forward_with_replacement(self, inputs, replace_index=None):
        ...
        if replace_index is not None:
            zero_vector = torch.zeros_like(inputs[replace_index])
            inputs = (
                inputs[:replace_index] + [zero_vector] + inputs[replace_index + 1 :]
            )
...
class ZeroVectorInteractionMoE(nn.Module):
    def __init__(
        self,
        num_modalities=3,
        fusion_model=None,
        ...
        super(ZeroVectorInteractionMoE, self).__init__()
        ...
        self.interaction_experts = nn.ModuleList(
            [
                ZeroVectorInteractionExpert(deepcopy(fusion_model), fusion_sparse)
