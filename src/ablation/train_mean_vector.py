...
from src.ablation.MeanVectorIMoE import MeanVectorInteractionMoE
...
def compute_mean_vectors(train_loader, encoder_dict, device):
    running_means = {mod: RunningMean() for mod in encoder_dict}

    with torch.no_grad():
        for batch_samples, *_ in train_loader:
            batch_samples = {k: v.to(device) for k, v in batch_samples.items()}
            for modality, samples in batch_samples.items():
                encoded = encoder_dict[modality](samples)
                running_means[modality].update(encoded)

    return {mod: rm.mean().detach() for mod, rm in running_means.items()}


def train_and_evaluate_mean_vector(args, seed, fusion_model, fusion):
    """Train and evaluate zero vector MoE.
    ...
    ensemble_model = MeanVectorInteractionMoE(
    ...
    for epoch in trange(args.train_epochs):
        ...
        for batch_samples, batch_labels, batch_mcs, batch_observed in train_loader:
            ...
            for i, (modality, samples) in enumerate(batch_samples.items()):
                ...
                running_means[modality].update(encoded_samples)
                mean_vec = running_means[modality].mean().detach()
                mean_vecs.append(mean_vec)

            if args.fusion_sparse:
                _, _, outputs, interaction_losses, gate_losses = ensemble_model(
                    fusion_input, mean_vectors= mean_vecs
                )
            else:
                _, _, outputs, interaction_losses = ensemble_model(fusion_input, mean_vectors= mean_vecs)
    ...
    if args.save:
        ...
        Path(f"./saves/mean_vector/{fusion}/{args.data}").mkdir(
            exist_ok=True, parents=True
        )

        if args.data == "mmimdb":
            save_path = f"./saves/mean_vector/{fusion}/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_f1_{best_val_f1:.2f}.pth"
        else:
            save_path = f"./saves/mean_vector/{fusion}/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_acc_{best_val_acc:.2f}.pth"
        torch.save(
    ...
    mean_vectors = compute_mean_vectors(train_loader, encoder_dict, device)
    ...
    visualize_sample_weights(
        all_routing_weights, args, framework="mean_vector", fusion=fusion
    )
