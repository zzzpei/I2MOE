...
from src.common.utils import (
    ...
    visualize_sample_weights_synergy_redundancy_only,
    visualize_expert_logits,
)

from src.ablation.SingleInteractionExpert import SingleInteractionExpert


def train_and_evaluate_single_interaction_expert(args, seed, fusion_model, fusion):
    """Train and evaluate single interaction expert.
    ...
    ensemble_model = SingleInteractionExpert(
        ...
        type_of_interaction=args.type_of_interaction,
    ).to(device)
    ...
    for epoch in trange(args.train_epochs):
        ...
        for batch_samples, batch_labels, batch_mcs, batch_observed in train_loader:
            ...
            if args.fusion_sparse:
                outputs, interaction_loss, expert_gate_loss = ensemble_model(
                    fusion_input
                )
            else:
                outputs, interaction_loss = ensemble_model(fusion_input)
        ...
        with torch.no_grad():
            for batch_samples, batch_labels, batch_mcs, batch_observed in val_loader:
                ...
                outputs = ensemble_model.inference(fusion_input)
    ...
    plot_total_loss_curves(
        ...
        framework="synergy_redundancy_only",
        fusion=fusion,
    )
    ...
    if args.save:
        ...
        Path(f"./saves/synergy_redundancy_only/{fusion}/{args.data}").mkdir(
            exist_ok=True, parents=True
        )

        if args.data == "mmimdb":
            save_path = f"./saves/synergy_redundancy_only/{fusion}/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_f1_{best_val_f1:.2f}.pth"
        else:
            save_path = f"./saves/synergy_redundancy_only/{fusion}/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_acc_{best_val_acc:.2f}.pth"
        torch.save(
    ...
    #     outputs,
    #     args,
    #     framework="single_intearction_expert",
    #     fusion=fusion,
    # )
    ...
    visualize_sample_weights_synergy_redundancy_only(
        all_routing_weights,
        args,
        framework=f"single_intearction_expert_{args.type_of_interaction}",
        fusion=fusion,
    )
