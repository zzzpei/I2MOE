...
from src.ablation.LessPerturbedForwardPassIMoE import LessPerturbedForwardPassIMoE


def train_and_evaluate_less_perturbed_forward(args, seed, fusion_model, fusion):
    ...
    ensemble_model = LessPerturbedForwardPassIMoE(
    ...
    plot_total_loss_curves(
        ...
        framework="less_perturbed_forward",
        fusion=fusion,
    )
    ...
    if args.save:
        ...
        Path(f"./saves/less_perturbed_forward/{fusion}/{args.data}").mkdir(
            exist_ok=True, parents=True
        )

        if args.data == "mmimdb":
            save_path = f"./saves/less_perturbed_forward/{fusion}/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_f1_{best_val_f1:.2f}.pth"
        else:
            save_path = f"./saves/less_perturbed_forward/{fusion}/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_acc_{best_val_acc:.2f}.pth"
        torch.save(
    ...
    visualize_sample_weights(
        all_routing_weights, args, framework="less_perturbed_forward", fusion=fusion
    )
