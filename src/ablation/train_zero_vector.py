...
from src.ablation.ZeroVectorIMoE import ZeroVectorInteractionMoE


def train_and_evaluate_zero_vector(args, seed, fusion_model, fusion):
    ...
    ensemble_model = ZeroVectorInteractionMoE(
    ...
    if args.save:
        ...
        Path(f"./saves/zero_vector/{fusion}/{args.data}").mkdir(
            exist_ok=True, parents=True
        )

        if args.data == "mmimdb":
            save_path = f"./saves/zero_vector/{fusion}/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_f1_{best_val_f1:.2f}.pth"
        else:
            save_path = f"./saves/zero_vector/{fusion}/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_acc_{best_val_acc:.2f}.pth"
        torch.save(
    ...
    visualize_sample_weights(
        all_routing_weights, args, framework="zero_vector", fusion=fusion
    )
