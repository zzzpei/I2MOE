"""Implements common fusion patterns. From MultiBench GitHub """
...
class MultiplicativeInteractions2Modal(nn.Module):
    ...
    def __init__(
        ...
        if output == "matrix3D":
            self.W = nn.Parameter(
                torch.Tensor(input_dims[0], input_dims[1], output_dim[0], output_dim[1])
            )
...
class NLgate(torch.nn.Module):
    ...
    def __init__(
        self, thw_dim, c_dim, tf_dim, q_linear=None, k_linear=None, v_linear=None
    ):
        """
