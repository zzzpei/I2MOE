...
from src.common.fusion_models.late_fusion import LateFusionTransformer
...
def parse_args():
    parser = argparse.ArgumentParser(description="lf")
...
def train_and_evaluate(args, seed):
    ...
    fusion_model = LateFusionTransformer(
        embed_dim=args.hidden_dim * len(args.modality),
        output_dim=n_labels
    ).to(device)
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/baseline/lf/{args.data}", f"{args.data}", f"{args.modality}.txt"
    )
