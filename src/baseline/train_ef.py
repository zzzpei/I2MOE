...
from src.common.fusion_models.early_fusion import EarlyFusionTransformer
...
def parse_args():
    parser = argparse.ArgumentParser(description="ef")
...
def train_and_evaluate(args, seed):
    ...
    fusion_model = EarlyFusionTransformer(
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/baseline/ef/{args.data}", f"{args.data}", f"{args.modality}.txt"
    )
