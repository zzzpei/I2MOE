...
def parse_args():
    parser = argparse.ArgumentParser(description="interpretcc")
    ...
    parser.add_argument("--num_layers_enc", type=int, default=256)
    parser.add_argument(
...
def train_and_evaluate(args, seed):
    ...
    if args.save:
        ...
        Path(f"./saves/baseline/interpretcc/{args.data}").mkdir(
            exist_ok=True, parents=True
        )

        if args.data == "mmimdb":
            save_path = f"./saves/baseline/interpretcc/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_f1_{best_val_f1:.2f}.pth"
        elif args.data == "mosi_regression":
            ...
            save_path = f"./saves/baseline/interpretcc/{args.data}/seed_{seed}_modality_{args.modality}_train_epochs_{args.train_epochs}_val_acc_{best_val_acc:.2f}.pth"
        torch.save(
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/baseline/interpretcc/{args.data}",
        f"{args.data}",
        f"{args.modality}.txt",
    )
